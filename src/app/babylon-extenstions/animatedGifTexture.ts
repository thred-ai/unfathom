 ...
 */
export class AnimatedGifTexture extends BABYLON.Texture {
  ...
  private _previousFrame: BABYLON.Nullable<GifFrame>;
  ...
  constructor(
    ...
    onLoad: BABYLON.Nullable<() => void> = null
  ) {
    super(url);
  ...
  private _createGifResources(): void {
    for (let frame of this._frames) {
      ...
      frame.texture = this._engine.createRawTexture(
        new Uint8Array(frame.patch.buffer),
        frame.dims.width,
        frame.dims.height,
        BABYLON.Constants.TEXTUREFORMAT_RGBA,
        false,
      ...
      const ty =
        (this._frames[0].dims.height - (frame.dims.top + frame.dims.height)) /
        this._frames[0].dims.height;
      frame.worldMatrix = new Float32Array([sx, 0, tx, 0, sy, ty, 0, 0, 1]);
    ...
    this._renderTarget = this._engine.createRenderTargetTexture(
      this._frames[0].dims,
      {
  ...
  private _renderFrame(): void {
    ...
    if (
      this._currentFrame &&
      BABYLON.PrecisionDate.Now - this._previousDate < this._currentFrame.delay
    ) {
      return;
    }
  ...
  private _drawPatch(): void {
    ...
    if (
      this._previousFrame &&
      (this._previousFrame.disposalType === 2 || this._nextFrameIndex === 0)
    ) {
      ...
      this._patchEffectWrapper.onApplyObservable.addOnce(() => {
        ...
        this._patchEffectWrapper.effect.setMatrix3x3(
          'world',
          this._previousFrame.worldMatrix
        );
        this._patchEffectWrapper.effect._bindTexture(
          'textureSampler',
          this._previousFrame.texture
        );
      });

      this._patchEffectRenderer.render(
        this._patchEffectWrapper,
        this._renderTarget
      );
    }
